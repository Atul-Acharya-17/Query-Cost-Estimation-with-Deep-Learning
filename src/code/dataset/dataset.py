...
class Table(object):
    def __init__(self, dataset, version):
        ...
        print(f"start building data {self.name}...")
...
def load_table(dataset: str, version: str, overwrite: bool=False) -> Table:
    ...
    if not overwrite and table_path.is_file():
        print("table exists, load...")
        with open(table_path, 'rb') as f:
            table = pickle.load(f)
        print(f"load finished: {table}")
        return table

    table = Table(dataset, version)
    print("dump table to disk...")
    dump_table(table)
    return table
...
def dump_table_to_num(dataset: str, version: str) -> None:
    ...
    print(f"dump csv file to {csv_path}")
...
if __name__ == '__main__':
    ...
    load_table(dataset=dataset, version=version, overwrite=True)
