...
class TreeLSTM(nn.Module):
    ...
    def __init__(self, op_dim, pred_dim , feature_dim, hidden_dim, hid_dim, embedding_type='tree_pool'):
        ...
        if embedding_type == 'tree_pool':
            self.lstm = nn.LSTM(4 * self.mlp_hid_dim, self.lstm_hidden_dim, batch_first=True)

        else:
            self.lstm = nn.LSTM(2 * self.lstm_hidden_dim + 2 * self.mlp_hid_dim, self.lstm_hidden_dim, batch_first=True)
...
class TreeLSTMBatch(nn.Module):

    def __init__(self, op_dim, pred_dim , feature_dim, hidden_dim, hid_dim, embedding_type='tree_pool'):

        super(TreeLSTMBatch, self).__init__()
    ...
    def tree_representation(self, nodes):
        ...
        if len(left_nodes) > 0:
            _, (left_hidden_state, left_cell_state) = self.tree_representation(left_nodes)
        ...
        if len(right_nodes) > 0:
            _, (right_hidden_state, right_cell_state) = self.tree_representation(right_nodes)
        ...
        return self.lstm(input_vector.view(batch_size, 1, -1), (hidden_state, cell_state))
    ...
    def forward(self, nodes, batch=True):
        ...
        if batch:
            batch_size = len(nodes)
            _, (hidden_state, _) = self.tree_representation(nodes)
